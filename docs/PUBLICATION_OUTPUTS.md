...
`publication/generated/`, and checks sentinel values for the Rossella/C1E outer
peak count and the default 1.4 cN/25 mm threshold total. Use the included
...
- Table 1 and Table 2 source values are provided in `publication/source_data/`.
  ...
  Table 2 includes the five canonicalized contact-angle measurements extracted
  from `gonio.xls` and the reported central-three summary values.
